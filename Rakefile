...
class Build
  ...
  def set_release_version_and_build_number
    ...
    doc.root.attributes['android:versionName'] = 1.3
    doc.root.attributes['android:versionCode'] = 9
...
namespace :generate do
  ...
  task :ws do
    ...
    ws_template_version = @params["ws_template_version"]
    spreadsheet_name = @params["spreadsheet_name"]
    package = @params["package"]

    definition_base = "#{ws_definitions_file_name}.xml"
    use_local_definition = File.file?(definition_base)
    base_dir = Dir.pwd
    ...
    Dir.chdir 'ws-gen' do
      `bundle install`
      `ruby generator.rb -f #{definition_path} -package #{package} -pn #{project_file_name} -aOutput #{generated_code_path} -aVersion #{ws_template_version}`
    end
